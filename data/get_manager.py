...
for i in range(len(tscodes)):
    ...
    manager_df=pd.DataFrame(res['data']['items'])[['personal_name','position_name']]
    manager_df['ts_code']=code
    df_list.append(manager_df)
    print(str(i)+" done")

result=pd.concat(df_list)
result=result.reset_index(drop=True)
result.to_sql('manager', con=engine,index=True,index_label='id', if_exists='append')