...
def request_data(code):
    ...
    params={'symbol':symbol,'circula':'0','count':'200'}
...
for i in range(len(tscodes)):
    ...
    if len(res['data'])==0:
        continue
    holder_df=pd.DataFrame(res['data']['items'])
    holder_df['hold_ts_code']=code
    holder_df=holder_df[[ 'holder_name', 'hold_ts_code','held_num', 'held_ratio']]
